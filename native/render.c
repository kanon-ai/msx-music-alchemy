...
int main(void) {
  ...
  PSG *psg=PSG_new(1789773,44100); OPLL *opll=OPLL_new(3579545,44100); SCC *scc=SCC_new(3579545,44100);
  if(!psg||!opll||!scc) return 3;
  PSG_setVolumeMode(psg,2); PSG_setQuality(psg,1); PSG_reset(psg);
  OPLL_reset(opll); OPLL_resetPatch(opll,0);
  ...
  for(unsigned f=0;f<frames;f++) {
    ...
    for(unsigned j=0;j<count;j++) {
      ...
      else if(chip==1) OPLL_writeReg(opll,reg,val);
      else if(chip==2) SCC_write(scc,0x9800+reg,val);
      else return 2;
    }
    for(unsigned i=0;i<44100/hz;i++) {
      int sample=(int)(PSG_calc(psg)*0.75+OPLL_calc(opll)*0.65+SCC_calc(scc)*0.65);
      if(sample>32767)sample=32767; if(sample< -32768)sample=-32768;
      putchar(sample&255); putchar((sample>>8)&255);
    }
  }
  PSG_delete(psg); OPLL_delete(opll); SCC_delete(scc); return ferror(stdout)?4:0;
}
