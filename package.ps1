...
python -m PyInstaller --noconfirm --clean --onedir --name MSXMusicAlchemy --add-data 'static;static' --add-data 'docs;docs' --add-data 'LICENSE;.' --add-data 'licenses;licenses' --add-data 'vendor/emu2149/LICENSE;licenses/emu2149' --add-data 'vendor/ymfm/LICENSE;licenses/ymfm' --add-data 'vendor/emu2212/LICENSE;licenses/emu2212' --add-binary 'bin/msx_render.exe;bin' launcher.py
...
Compress-Archive -Path dist/MSXMusicAlchemy -DestinationPath releases/MSXMusicAlchemy-v0.5.1-windows-x64.zip -Force
Get-FileHash releases/MSXMusicAlchemy-v0.5.1-windows-x64.zip -Algorithm SHA256
